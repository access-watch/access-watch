...
class Database {
  ...
  gc () {
    ...
    this.indexes = this.indexes.map(
      (indexes, type) => indexes.map((index, key) => {
        if (key === '24h') {
        ...
        return index.sort().reverse().slice(0, 1000)
      })
    )
  }
  ...
  index (session) {
    this.indexes = this.indexes.updateIn([session.get('type'), '24h', session.get('id')], 0, n => n + 1)
    this.indexes = this.indexes.updateIn([session.get('type'), '15m', session.get('id')], 0, n => n + 1)
  }
  ...
  list ({type, sort, filter, limit}) {
    ...
    sessions = index.keySeq().map(key => this.get(type, key))
    if (filter) {
