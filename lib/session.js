...
const { iso, now, dropWhile } = require('./util')
...
class Speed {
  ...
  compute () {
    ...
    let speed = Range(0, this.size).map(n => this.counters[currentMinute - n * 60])
    speed = dropWhile(speed.reverse(), v => v === undefined)
    return speed.reverse().map(v => (v === undefined) ? 0 : v)
  }
}
