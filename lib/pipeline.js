  ...
  return (stream) => {
    return (event) => {
      ...
      windows = windows.updateIn([id, event.get('key')], Map(), windows => {
        ...
        // merge windows
        windows = Map(strategy
                      .merge(windows.valueSeq(),
                             (w1, w2) => {
                               return w1.merge(w2).set('acc', reducer.merge(w1.get('acc'), w2.get('acc')))
                             })
                      .map(window => [window.get('start') + ':' + window.get('end'), window]))
...
function store () {
  return (stream) => {
    return (event) => {
      db.add(event.get('data').set('time', event.get('time')))
      return event
    }
  }
}
...
/**
 * Augment event with session data.
 */
function augmentWithSession (f) {
  return (stream) => {
    return (event) => {
      let sess = f(event.get('data'))
      if (sess) {
        forward(stream, event.setIn(['data', 'session'], session.ensure(sess)))
      } else {
        // HACK: Remove potential session data to avoid conflict
        forward(stream, event.removeIn(['data', 'session']))
      }
    }
  }
}
 ...
 * Update a session
 */
function sessionUpdate (f) {
  return (stream) => {
    return (event) => {
      session.update(event.get('key'), session => {
        const update = f(event.get('data'))
        return update(session).set('updated', event.get('time'))
      })
      forward(stream, event)
    }
  }
...
class Builder {
  ...
  session (f) {
    return this
      .add(augmentWithSession(f))
      .by(log => log.getIn(['session', 'id']))
  }

  sessionUpdate (f) {
    return this.add(sessionUpdate(f))
  }
...
class Pipeline extends Builder {
  ...
  // TODO: Close inputs properly
  close () {
    console.log('Stopping the pipeline.')
  }
}
