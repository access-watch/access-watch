...
exports.loadingCache = ({size, maxAge, load, onError}) => {
  ...
  return (key) => {
    ...
    if (typeof (key) === 'object') {
      if (!key.cacheKey) {
        throw new Error('If the key is an object, Set the `cacheKey` property on it.')
      } else {
...
/**
 * Drop collection elements while the predicate is true.
 */
exports.dropWhile = (coll, pred) => {
  if (coll.isEmpty()) {
    return coll
  }
  if (pred(coll.get(0))) {
    return exports.dropWhile(coll.rest(), pred)
  }
  return coll
}
