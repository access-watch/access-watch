...
const { gc: gcConfig } = config.rules
...
class Database {
  ...
  gc () {
    const cutoff = now() - gcConfig.expiration
  ...
  static deserialize (data) {
    ...
    if (data) {
      db.rules = fromJS(data.rules).map(rule => {
        return rule
          .updateIn(['blocked', 'per_minute'], speed => Speed.deserialize(speed.toJS()))
          .updateIn(['blocked', 'per_hour'], speed => Speed.deserialize(speed.toJS()))
          .updateIn(['passed', 'per_minute'], speed => Speed.deserialize(speed.toJS()))
          .updateIn(['passed', 'per_hour'], speed => Speed.deserialize(speed.toJS()))
      })
    }
    return db
  }
