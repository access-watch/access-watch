...
function compileLineParser(format) {
  ...
  const matchString = ([head, ...tail], text) => {
    if (!text.startsWith(head)) {
      throw new Error(
        `Syntax error. Was expecting the string ${head}. Got: ${text}`
      );
    }
    if (tail.length === 0) {
      return Map();
  ...
  const matchVariable = ([head, ...tail], text) => {
    ...
    if (len === -1) {
      throw new Error(
        `Syntax error. Was expecting the variable ${head}. Got ${text}.`
      );
    }
...
const transformers = {
  ...
  '%r': (log, value) =>
    log.update('request', request => parseRequest(value).merge(request)),
};
